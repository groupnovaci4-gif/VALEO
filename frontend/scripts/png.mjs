// VALEO — codec PNG minimal, sans aucune dépendance.
//
// Extrait de `build-logo.mjs` pour être partagé avec `build-icons.mjs` : les
// deux scripts décodent le même logo source, le réduisent et le réencodent.
// Une seconde implémentation aurait divergé au premier correctif — et un
// défaut de décodage y serait passé inaperçu deux fois plutôt qu'une.
//
// Volontairement limité à ce dont VALEO a besoin : PNG 8 bits par canal, non
// entrelacé. Le refus est explicite plutôt que silencieux (voir `decoderPNG`).
...
export const CANAUX = { 0: 1, 2: 3, 3: 1, 4: 2, 6: 4 };
...
export function decoderPNG(buf) {
  ...
  if (ihdr.profondeur !== 8)
    throw new Error(`Profondeur ${ihdr.profondeur} bits non gérée : exporter le logo en 8 bits par canal.`);
  if (ihdr.entrelace !== 0)
    throw new Error("PNG entrelacé (Adam7) non géré : réexporter sans entrelacement.");
  ...
  const px = defiltrer(inflateSync(Buffer.concat(idat)), ihdr.largeur, ihdr.hauteur, CANAUX[ihdr.couleur]);
  return { rgba: versRGBA(px, ihdr, plte, trns), largeur: ihdr.largeur, hauteur: ihdr.hauteur, ihdr };
}

export { lireChunks, defiltrer, versRGBA, reduireSurBlanc, encoderPNG };
